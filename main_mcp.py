...
# --- MCP tool: Agent-based contact aggregation (Gmail only)
...
# --- MCP tool: EmailFetcherAgent for IMAP
@mcp.tool(name="imap_fetch_latest_unread")
def imap_fetch_latest_unread(imap_server: str, email_user: str, email_pass: str, n: int = 5):
    agent = EmailFetcherAgent(imap_server, email_user, email_pass)
    return agent.fetch_latest_emails(n)
